          ...
          <div className="hidden md:flex items-center space-x-8">
            {links.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="font-title text-xl font-bold text-slate-900 hover:text-[#ff7300] hover:underline decoration-4 underline-offset-8 decoration-[#0969a7] transition-all uppercase tracking-wide"
              >
                {link.name}
              </a>
            ))}
